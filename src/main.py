...
df.to_csv("luftkvalitet_byer.csv", index=False)
...
bars = plt.bar(df["By"], df["AQI"], color=farger)
...
for bar, pollutant in zip(bars, df["Dominerende forurensning"]):
    if bar.get_height():
...
plt.ylim(0, max(df["AQI"].dropna()) + 20)
